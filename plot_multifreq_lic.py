...
# seed = enmap.smooth_gauss(seed, 2*u.arcmin*u.fwhm)
seed = None
imap = enmap.smooth_gauss(imap, 3.5*u.arcmin*u.fwhm)
...
if not op.exists(args.texture) or args.force:
    ...
    texture = lib.LIC_texture(theta, length=0.25, seed=seed, contrast=True)
    np.save(args.texture, texture)
else:
    texture = np.load(args.texture)
textures = np.stack([np.ones_like(texture)*P/P.max()]*3+[texture*0.35], axis=2)
